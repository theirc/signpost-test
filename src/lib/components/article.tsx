...
export function Article() {
  ...
  return <div className={`overflow-y-auto ${isRTL ? 'rtl' : ''}`}>
    <Container className="text-black bg-white" id="main-content">
      <div className={`flex flex-col ${isRTL ? 'text-right' : ''}`} ref={refScrollUp}>
        <Breadcrumb className="rs_skip" separator=">" items={[{ title: <a href="/">{translate(translations.home)}</a> }, { title: <a href={`/${locale.toLowerCase()}/categories/${category.id}`}>{translate(category.name)}</a> }, { title: <a href={`/${locale.toLowerCase()}/categories/${category.id}/${section.id}`}>{translate(section.name)}</a> }, { title: translate(a.name) }]} />
        <div>
          ...
          <div className={`flex items-center mt-4 sm:flex-row flex-col ${isRTL ? 'flex-row-reverse' : ''}`}>
