...
async function execute(worker: MessageWorker) {
  ...
  try {
     ...
     const messageParts = content.split('<break>');
     ...
     await sendTelerivetMessage(worker, toNumber, cleanMessage(content), routeId, quickReplies, mediaUrls);
   } catch (error: any) {
 ...
 async function sendTelerivetMessage(
   ...
   if (quickReplies && quickReplies.length > 0) {
    ...
    const isBrowser = typeof window !== 'undefined'
              ...
              if (isBrowser) {
       // Use the proxy API to avoid CORS issues - use Basic Auth with API key as username for /messages/send
       console.log(`${logPrefix} - Message Payload:`, messagePayload)
       ...
       const proxyPayload = {
         ...
         timeout: 10000,
         authType: 'basic',
         username: worker.parameters.telerivetApiKey,
         selectedKeyName: 'telerivetApiKey'
       }
       ...
       const response = await fetch(telerivetUrl, {
         ...
         headers: {
           'Content-Type': 'application/json',
           'Authorization': `Basic ${btoa(`${worker.parameters.telerivetApiKey}:`)}`
         },
