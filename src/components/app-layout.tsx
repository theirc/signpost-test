...
import { Routes, Route, useLocation, useNavigate, useMatch, BrowserRouter } from "react-router-dom"
...
function NavigationLink({ to, children }: { to: string; children: React.ReactNode }) {
  ...
  return <BreadcrumbLink onClick={() => navigate(to)}>{children}</BreadcrumbLink>;
}
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col p-2 pt-0">
          <Routes>
            ...
            <Route path="/settings/team/users/:id" element={<UserForm />} />
